...
export class CrearUsuarioUseCase implements UseCase<NewUser, User> {
  ...
  async execute(input: NewUser): Promise<User> {
    ...
    const existing = await this.#users.findByNombre(parsed.nombre, parsed.businessId);
    if (existing) {
      throw new TypeError(`Ya existe un usuario con el nombre "${parsed.nombre}"`);
    }
  ...
  #pickColor(): string {
    const colors = ['blue', 'green', 'red', 'purple', 'cyan', 'slate'] as const;
