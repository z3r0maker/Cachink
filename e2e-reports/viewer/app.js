...
(function () {
  ...
  function renderFilterChips() {
    ...
    container.innerHTML = chips
      .map(
        (c) => `<button class="chip ${c.cls}" data-key="${c.key}" data-type="${c.type}">${c.label}</button>`
      )
      .join("");
  ...
  function renderStepTrace(test) {
    ...
    tbody.innerHTML = test.steps
      .map((s, i) => {
        ...
        const rowCls = status === "FAILED" ? "step-failed" : status === "WARNED" ? "step-warned" : "";
  ...
  function renderExpectedActual(test) {
    ...
    if (hier.ids && hier.ids.length > 0) {
      actualHtml += hier.ids
        ...
        .join("");
      if (hier.ids.length > 20) actualHtml += `<div class="ea-item" style="color:#64748b">... y ${hier.ids.length - 20} más</div>`;
    }
    if (hier.texts && hier.texts.length > 0) {
      ...
      actualHtml += hier.texts
        ...
        .join("");
      if (hier.texts.length > 15) actualHtml += `<div class="ea-item" style="color:#64748b">... y ${hier.texts.length - 15} más</div>`;
      actualHtml += "</div>";
    }
    actualHtml += "</div>";
