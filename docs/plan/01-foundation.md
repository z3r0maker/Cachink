...
- **Files:** every `package.json` `name` + `dependencies` under `packages/*`, `apps/mobile`, root; every `import … from '@cachink/…'`; `tsconfig*.json` `paths`; `eslint.config.js`; `turbo.json` filters; `vitest.config.*` aliases; `.storybook/main.ts`; docs that show commands (`README.md`, `SETUP.md`, `docs/archive/e2e-HANDOFF.md`).
- **Steps:**
