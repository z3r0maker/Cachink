...
the link's **fragment** — `https://app.xangarro.mx/activar#c=<token>` — so no server log or
Referer ever carries it, and nothing redeems on `GET /activar`. Redeeming it consumes the code's
row: one pairing, whichever path the phone used. Typed path:
