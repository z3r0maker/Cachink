...
| Vercel projects  | repo `z3r0maker/Cachink`: **portal** (root `apps/portal`, `app.xangarro.mx`), **admin** (root `apps/admin`, `admin.xangarro.mx`) |
...
| `ENTITLEMENT_PRIVATE_KEY` + public key                                     | `pnpm --filter @xangarro/portal entitlement:keygen` | portal / mobile EAS env       |
...
format `apps/portal/src/server/device/credentials.ts` reads) for the portal
...
4. **Migrations** — ledger table `xangarro_ops.migrations` (name, checksum,
   ...
   then `apps/admin/src/server/db/migrations/*.sql`. Each file and its ledger row
...
- **Staff** (the console cannot create staff; ADR-080). With the **postgres**
  ...
  read -rs PASSWORD && printf %s "$PASSWORD" | \
    DATABASE_URL='<SUPERUSER_URL from §3>' pnpm --filter @xangarro/admin staff create \
...
--filter @xangarro/portal stripe:seed` creates the products, the four prices
...
and its meaning is in `apps/portal/.env.example` and `apps/admin/.env.example`
