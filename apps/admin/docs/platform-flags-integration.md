...
## 2. Reading the rows (apps/portal)

`apps/portal/src/server/device/bootstrap.ts:71` `entitlementFor(businessId, now)`
...
- The view filters each allowlist down to `xangarro.current_business_id()`,
  ...
  `apps/portal/src/server/sync/pull.ts:40` (already in the tenant tx),
  `apps/portal/src/server/device/activate.ts:94` (after `set_config`, `:93`),
  and `apps/portal/src/app/api/v1/entitlement/route.ts:18` (must open the
...
- `asesorLlm` replaces ADR-059's environment gate at
  `apps/portal/src/app/(portal)/asesor/screen.tsx:21`
