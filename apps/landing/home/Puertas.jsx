...
const PUERTAS = [
  ...
  },
  {
    ...
    tag: 'Hoy',
    tagBg: 'var(--green-soft)',
    title: 'La caja del mostrador',
  ...
  },
  {
    ...
    tile: 'var(--blue-soft)',
    tag: 'Próximamente',
    tagBg: 'var(--warning-soft)',
    title: 'La app del equipo',
  ...
  return (
    <div className={p.main ? 'xcard puerta puerta-main' : 'xcard puerta'}>
      <div className="puerta-top">
        ...
        <span className="xtag" style={{ background: p.tagBg }}>
          {p.tag}
        </span>
      </div>
      <h3>{p.title}</h3>
      ...
      <div className="puerta-chips">
        {p.chips.map((c) => (
          <span
            key={c}
            className="xtag"
            style={{ background: p.main ? 'var(--white)' : 'var(--gray-100)' }}
          >
            {c}
          </span>
        ))}
      ...
      <div className="xh-wrap">
        <div className="xhead">
          ...
          <p className="xlead" style={{ maxWidth: 420, color: 'var(--gray-600)' }}>
            Tú manejas el negocio desde el portal. Tu equipo cobra desde la computadora del negocio,
            la tablet del mostrador o el teléfono. Todo cae en el mismo lugar, con los mismos
            números.
          </p>
        </div>
