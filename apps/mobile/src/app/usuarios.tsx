...
export default function UsuariosRoute(): ReactElement {
  ...
  function handleConfirmDelete(user: User, onConfirmed: () => void): void {
    Alert.alert('Eliminar usuario', `¿Seguro que deseas eliminar a ${user.nombre}?`, [
  ...
  return (
    <AppShellWrapper activeTabKey="otros" onBack={() => router.back()}>
      <UsuariosContent onConfirmDelete={handleConfirmDelete} testID="usuarios-route" />
    </AppShellWrapper>
  );
}
