 ...
 *
 * Renamed from /inventario. Renders Stock or Movimientos depending on
 * the inner tab state. Stock rows open ProductoDetailRoute. The
 * "+ Nuevo Producto" Btn opens NuevoProductoModal wired to useCrearProducto.
 */
...
function ProductosBody(props: BodyProps): ReactElement {
  return props.tab === 'stock' ? props.stockSlot : <MovimientosRoute />;
}

function useProductosState(): {
  query: string;
  ...
  return {
    query, setQuery, tab, setTab, modalOpen, setModalOpen,
    selected, setSelected, swipe, itemsQ, crear, filtered,
  };
}

export default function ProductosRoute(): ReactElement {
  const s = useProductosState();
  const stockSlot = (
    <StockSlot
      query={s.query}
  ...
  return (
    <AppShellWrapper activeTabKey="productos">
      <InventarioTabBar active={s.tab} onChange={s.setTab} />
      <ProductosBody tab={s.tab} stockSlot={stockSlot} />
      <NuevoProductoModal
        open={s.modalOpen}
