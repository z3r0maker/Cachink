...
import {
  ...
  type ImportResult,
  type PreviewRow,
} from '@/server/actions/importar-productos';

/**
 * The import's three steps (P-07): pick a file → preview → import. The file
 * stays in the browser and is sent again on commit; the server re-plans it
...
export function useImportar() {
  ...
  const run = (fn: (f: File) => Promise<void>) => {
    if (file === null) return setError('Elige un archivo .xlsx.');
    setError(null);
    startTransition(() => fn(file));
  };
  const revisar = () =>
    run(async (f) => {
      const r = await previsualizarImportacion(asForm(f));
      if (r.ok) setStep({ name: 'revision', rows: r.rows });
      else setError(r.message);
    });
  const importar = () =>
    run(async (f) => {
      const r = await importarProductos(asForm(f));
