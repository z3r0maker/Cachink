...
test(
  'approving a product needs cost, category and stock, and writes them',
  { tag: SERIAL_TAG },
  async ({ page }) => {
    await page.goto('/revision-caja');
...
test(
  'a client is approved with a limit and a term, both stored',
  { tag: SERIAL_TAG },
  async ({ page }) => {
    await page.goto('/revision-caja');
...
test(
  'the duplicate merges: its row says where its facts went',
  { tag: SERIAL_TAG },
  async ({ page }) => {
