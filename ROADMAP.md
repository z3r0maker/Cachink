...
**Current phase:** Phase 0 — Foundation (ready to close — 97% complete)
**Next unblocked task:** Phase 0 closeout — archive milestones M1–M7 to `ROADMAP-archive.md`, collapse to a 3-line summary, delete `SCAFFOLD-STATUS.md`, open Phase 1A (Storybook-vs-Ladle ADR).
**Blocked on user / environment (not on Phase-0 closeout — these are follow-ups):**
- **Full Xcode** to run `pnpm ios` (App Store install, ~15 GB). `pnpm web` verified the identical bundle renders.
- **Android Studio** to run `pnpm android`.
- **Empty PR for CI smoke check** (P0-M7-T03) — repo not yet a git remote; rolls up into Phase 1A prep.
...
- [ ] **P0-M7-T02** Add Renovate config for weekly dependency PRs
- [ ] **P0-M7-T03** Verify CI green on an empty PR (blocked: repo not yet a git remote)
...
- [ ] **P1A-M1-T01** Pick Storybook 8 vs Ladle (decision → ADR)
- [ ] **P1A-M1-T02** Set up the chosen tool in `packages/ui` with native + web targets
- [ ] **P1A-M1-T03** Add a CI job that runs visual regression tests on both targets

### Milestone P1A-M2 — Core primitives (CLAUDE.md §8.4)
- [ ] **P1A-M2-T01** `Btn` with all 6 variants + press animation
- [ ] **P1A-M2-T02** `Input` (text, number, date, select) with label, note, placeholder
- [ ] **P1A-M2-T03** `Tag`
- [ ] **P1A-M2-T04** `Modal` (bottom-sheet on mobile, centered on desktop — platform-extension pattern)
- [ ] **P1A-M2-T05** `EmptyState`
- [ ] **P1A-M2-T06** `SectionTitle`
- [ ] **P1A-M2-T07** `Card` (white, yellow, black variants)
- [ ] **P1A-M2-T08** `Kpi`
- [ ] **P1A-M2-T09** `Gauge`
