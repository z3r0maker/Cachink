# AGENTS.md — Cachink! Project Contract
...
1. **Read this file (AGENTS.md) fully.** These are the architectural rules that always apply.
...
| ----------------- | -------------------------------- | ----------------------------- | ----------------------------- |
| `AGENTS.md`       | Architectural contract (rules)   | Humans only, via ADR          | Grows; never shrinks          |
...
**Never edit AGENTS.md to mark progress.** Progress lives in ROADMAP.md.
**Never remove rules from AGENTS.md.** If a rule must change, add an ADR in ARCHITECTURE.md first.
...
10. **AGENTS.md is the architectural contract, not a task list.** It grows only when new rules are added. It is never rewritten or shrunk by agents. Implementation progress is tracked in `ROADMAP.md`. Significant architectural decisions are logged as ADRs in `ARCHITECTURE.md`.
...
- **Domain + Application layers:** Every new entity test and use-case test
  is mandatory per AGENTS.md §2.4 (TDD). Minimum 1 happy path + 3 unhappy
  paths per use case.
...
No agents deployed. Create new agents in `.Codex/agents/` as ADRs require specialized guidance.
...
No skills deployed. Reference AGENTS.md rules directly. For architecture questions, see `ARCHITECTURE.md`.
