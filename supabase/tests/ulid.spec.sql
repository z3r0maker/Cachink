...
--
-- Runs via `supabase test db`. Verifies `xangarro_generate_ulid()`
-- returns a string that the Xangarro domain layer's regex will accept:
...
-- 1) Length is 26.
SELECT is(
  length(public.xangarro_generate_ulid()),
  26,
...
SELECT ok(
  public.xangarro_generate_ulid() ~ '^[0-9A-HJKMNP-TV-Z]{26}$',
  'ULID matches the Crockford base32 alphabet'
);

-- 3) Is never NULL.
SELECT is(
  public.xangarro_generate_ulid() IS NULL,
  FALSE,
  'ULID is never NULL'
);
