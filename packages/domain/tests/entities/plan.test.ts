...
describe('plan', () => {
  ...
  it('freelancer is capped at 50 records/month; paid plans are unlimited', () => {
    expect(PLAN_LIMITS.freelancer.recordsPerMonth).toBe(50);
    expect(PLAN_LIMITS.emprendedor.recordsPerMonth).toBeNull();
    expect(PLAN_LIMITS.mipyme_pro.recordsPerMonth).toBeNull();
  });
  ...
  it('every plan feature is a known flag key and higher plans are supersets', () => {
    ...
    for (const f of PLAN_LIMITS.emprendedor.features) {
      expect(PLAN_LIMITS.mipyme_pro.features).toContain(f);
    }
  });

  it('rejects an unknown plan id and falls back to freelancer', () => {
    expect(() => PlanIdSchema.parse('enterprise')).toThrow();
    expect(FALLBACK_PLAN).toBe('freelancer');
  });
});
