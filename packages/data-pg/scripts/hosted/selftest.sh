...
for dir in packages/data-pg/hosted packages/data-pg/drizzle apps/backoffice/src/server/db/migrations; do
  mkdir -p "$TREE/$dir" && cp "$REPO/$dir"/*.sql "$TREE/$dir/"
done
TOTAL=$(ls "$TREE"/packages/data-pg/hosted/*.sql "$TREE"/packages/data-pg/drizzle/*.sql \
  "$TREE"/apps/backoffice/src/server/db/migrations/*.sql | wc -l | tr -d ' ')
