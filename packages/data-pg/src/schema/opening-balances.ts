...
export const openingBalances = pgTable(
  ...
  (t) => [index('opening_balances_business_idx').on(t.businessId)],
);

/** One saldo inicial per cliente; the third fact of `estadoDeCuenta` (C-20). */
export const openingBalanceClients = pgTable(
  ...
  (t) => [index('opening_balance_clients_business_idx').on(t.businessId)],
);
