...
export const cajaMovimientos = pgTable(
  ...
  (t) => [index('caja_movimientos_business_idx').on(t.businessId)],
);

export const cajaTurnos = pgTable(
  'caja_turnos',
  {
    id: text('id').primaryKey(),
    userId: text('user_id').notNull(),
  ...
  (t) => [index('caja_turnos_business_idx').on(t.businessId, t.fecha)],
);

export const cancelacionLogs = pgTable(
  'cancelacion_logs',
  {
    id: text('id').primaryKey(),
  ...
  (t) => [index('cancelacion_logs_business_idx').on(t.businessId)],
);
...
export const dayCloses = pgTable(
  ...
  (t) => [index('day_closes_business_idx').on(t.businessId, t.fecha)],
);
