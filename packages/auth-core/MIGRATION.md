...
| --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/portal/src/server/actions/auth.ts:36-37` `DUMMY_HASH`                                                                 | `DUMMY_HASH` (`src/password.ts`) — same hash                                                                                                                                                               |
| `apps/portal/src/server/actions/auth.ts:10,44-48` `compare` + `passwordMatches`                                             | `verifyPassword(password, user?.hash ?? null)` — dummy compare built in; also rejects a malformed stored hash without throwing                                                                             |
| `apps/portal/src/server/actions/auth.ts:55-68` wait → compare → fail/clear                                                  | optional: `guardAttempt(throttleStore(db()), subjects, attempt)` (`src/throttle.ts`) — same order: refuse while locked, fail every subject, clear subjects marked `clearOnSuccess` (the email, not the IP) |
| `apps/portal/src/server/throttle-policy.ts:14-35` `LOGIN_PER_EMAIL`, `LOGIN_PER_IP`, `ACTIVATE_PER_IP`, `ACTIVATE_PER_CODE` | same names (`src/policies.ts`), same values (tested)                                                                                                                                                       |
| `apps/portal/src/server/throttle-policy.ts:38` `DEVICE_CALLS_PER_MINUTE`                                                    | same name (`src/policies.ts`)                                                                                                                                                                              |
| `apps/portal/src/server/throttle-policy.ts:45-47` `clientIp`                                                                | `clientIp` (`src/policies.ts`) — same behaviour; takes anything with `get()`                                                                                                                               |
| `apps/portal/src/server/throttle-policy.ts:49` `minutes`                                                                    | `minutes` (`src/policies.ts`)                                                                                                                                                                              |
...
After the switch `apps/portal/src/server/throttle-policy.ts` can be deleted
...
`apps/portal/src/server/session.ts` needs no change: it calls data-pg's
