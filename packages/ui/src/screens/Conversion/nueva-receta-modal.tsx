...
export function NuevaRecetaModal(props: NuevaRecetaModalProps): ReactElement {
  ...
  const [state, setState] = useState<FormState>({
    mpId: '', prodId: '', cantOrigen: '1', cantResultante: '1',
  });
  ...
  const handleSubmit = (): void => {
    ...
    setState({ mpId: '', prodId: '', cantOrigen: '1', cantResultante: '1' });
  };
  ...
  return (
    <Modal open={props.open} onClose={props.onClose}
      ...
      <Combobox label={t('conversion.mpLabel')} value={state.mpId}
        onChange={(v) => setState((p) => ({ ...p, mpId: v }))}
        options={mpOptions} testID="receta-mp-picker" />
      {errors.mp && <ErrorText msg={errors.mp} />}
      <Combobox label={t('conversion.productoLabel')} value={state.prodId}
        onChange={(v) => setState((p) => ({ ...p, prodId: v }))}
        options={prodOptions} testID="receta-prod-picker" />
      ...
      <IntegerField label={t('conversion.cantidadOrigenLabel')} value={state.cantOrigen}
        onChange={(v) => setState((p) => ({ ...p, cantOrigen: v }))}
        min={1} error={errors.cantOrigen} testID="receta-cant-origen" />
      <IntegerField label={t('conversion.cantidadResultanteLabel')} value={state.cantResultante}
        onChange={(v) => setState((p) => ({ ...p, cantResultante: v }))}
        min={1} error={errors.cantResultante} testID="receta-cant-resultante"
