...
describe('bannerKind', () => {
  it('is quiet when active, unverified, loading or a Freelancer plan', () => {
    expect(bannerKind(BASE)).toBeNull();
    expect(bannerKind({ ...BASE, verified: false, state: 'lapsed' })).toBeNull();
    expect(
      bannerKind({ ...BASE, plan: 'freelancer', grantedPlan: 'freelancer', state: 'lapsed' }),
    ).toBeNull();
  ...
  it('warns during grace and when a paid plan fell back', () => {
    ...
    expect(bannerKind({ ...BASE, plan: 'freelancer', state: 'lapsed' })).toBe('fellBack');
  });
});
  ...
  it('opens from a PlanLimitError on any mutation instead of an error toast', async () => {
    ...
    await act(async () => {
      await qc
        ...
        .build(qc, {
          mutationFn: async () => {
            throw new PlanLimitError('freelancer', 50);
          },
        })
        .execute(undefined)
        .catch(() => undefined);
