...
describe('useFeatureFlags', () => {
  ...
  it('keeps plan features off without a verified entitlement (Freelancer)', async () => {
  ...
  it('turns stock and barcode on for an Emprendedor entitlement', async () => {
    const { result } = renderHook(() => useFeatureFlags(), {
      wrapper: await setup({ plan: 'emprendedor' }),
    });
  ...
  it('respects a tenant toggle the portal turned off, even on a plan that includes it', async () => {
    const { result } = renderHook(() => useFeatureFlags(), {
      wrapper: await setup({ plan: 'emprendedor', tenant: { stock: false } }),
    });
