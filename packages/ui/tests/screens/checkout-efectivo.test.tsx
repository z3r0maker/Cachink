...
describe('CheckoutEfectivo', () => {
  it('renders without emoji characters', () => {
    const { container } = renderWithProviders(
      <CheckoutEfectivo totalCentavos={46600n} onConfirm={vi.fn()} />,
    );
  ...
  it('shows the total amount in the header', () => {
    renderWithProviders(<CheckoutEfectivo totalCentavos={46600n} onConfirm={vi.fn()} />);
    expect(screen.getByText('Total a cobrar')).toBeInTheDocument();
  });

  it('hides cash warning when efectivoEnCaja is not provided', () => {
    const { container } = renderWithProviders(
      <CheckoutEfectivo totalCentavos={10000n} onConfirm={vi.fn()} />,
    );
    expect(container.textContent?.includes('suficiente cambio')).toBe(false);
  });

  it('hides cash warning when efectivoEnCaja is null', () => {
    const { container } = renderWithProviders(
      <CheckoutEfectivo totalCentavos={10000n} onConfirm={vi.fn()} efectivoEnCaja={null} />,
    );
    expect(container.textContent?.includes('suficiente cambio')).toBe(false);
  });
