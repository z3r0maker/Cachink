...
async function sale(productoId: ProductId, over: Record<string, unknown> = {}) {
  // ADR-073: a line needs its ticket; make one per sale.
  folio += 1;
  const ticket = await new DrizzleTicketsRepository(db, DEV).create({
    folio,
    fecha: '2026-09-16',
    concepto: 'x',
    metodo: 'Efectivo',
  ...
  return new DrizzleSalesRepository(db, DEV).create(
    makeNewSale({ businessId: BIZ, productoId, ticketId: ticket.id, ...over } as never),
  );
}
...
describe('purgeAcknowledged', () => {
  ...
  it('keeps unpaid credit sales with their payments, and rows with unpushed edits', async () => {
    const pid = await product();
    const credito = await sale(pid, { metodo: 'Crédito' });
    // ADR-073/074: the fiado state is on the ticket; the abono is per client.
    await db.run(
      sql`UPDATE tickets SET estado_pago = 'parcial', cliente_id = ${'01HZ8XQN9GZJXV8AKQ5X0C7CKJ'} WHERE id = ${credito.ticketId}`,
    );
    const pago = await new DrizzleClientPaymentsRepository(db, DEV).create(
      makeNewClientPayment({
        businessId: BIZ,
        clienteId: '01HZ8XQN9GZJXV8AKQ5X0C7CKJ' as never,
      }),
    );
  ...
  it('keeps an open caja turno and a closed one still referenced by a kept sale', async () => {
    ...
    const credito = await sale(pid, { metodo: 'Crédito', cajaTurnoId: 'T-USED' });
    await db.run(
      sql`UPDATE tickets SET estado_pago = 'pendiente', caja_turno_id = 'T-USED' WHERE id = ${credito.ticketId}`,
    );
