...
describe('verifyEntitlement', () => {
  it('accepts a valid signature', () => {
    expect(verifyEntitlement(sign(PAYLOAD), PUB)?.plan).toBe('xangarro');
  });

  it('rejects a tampered payload', () => {
    const signed = sign(PAYLOAD);
    expect(
      verifyEntitlement({ ...signed, payload: { ...PAYLOAD, plan: 'xangarrote' } }, PUB),
    ).toBeNull();
  });
...
describe('resolveEntitlement', () => {
  ...
  it('keeps the plan during the 7-day payment grace, then falls to Xangarrito', () => {
  ...
  it('a device clock set forward cannot extend a plan', () => {
    expect(resolve({ deviceNow: at('2028-01-01T00:00:00.000Z') }).plan).toBe('xangarrito');
  });
