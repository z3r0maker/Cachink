...
describe('drainPush', () => {
  ...
  it('pushes a rung sale once, marks it accepted, and advances the cursor', async () => {
    const saleId = await ringSale(d);
    const first = await drainPush(deps(d));
    assert.deepEqual([first.accepted, first.rejected, first.error], [1, 0, null]);
...
describe('SyncEngine', () => {
  it('shares one in-flight run between concurrent callers and reports revocation', async () => {
    ...
    assert.equal(a, b);
    assert.equal(a.push?.accepted, 1);
