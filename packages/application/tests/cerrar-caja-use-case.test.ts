...
describe('CerrarCajaUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new CerrarCajaUseCase(turnos, tickets, sales, expenses);
  });
  ...
  it('accounts for sales in the expected cash computation', async () => {
    ...
    const __t = await tickets.create(
      makeNewTicket({ businessId: BIZ, metodo: 'Efectivo', fecha: '2026-05-09' }),
    );
    await sales.create(
  ...
  it('computes esperado from apertura + adicional + ventas - egresos', async () => {
    ...
    const __t3 = await tickets.create(
      makeNewTicket({ businessId: BIZ, metodo: 'Transferencia', fecha: '2026-05-09' }),
    );
    await sales.create(
