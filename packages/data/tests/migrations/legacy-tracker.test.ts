...
function oldRunnerInstall(): { sqlite: Database.Database; db: XangarroDatabase } {
  ...
  for (const tag of PRE_RENAME) insert.run(tag, NOW);
  sqlite.exec("INSERT INTO __cachink_sync_state (scope, value) VALUES ('localPushHwm', '7')");
...
describe('runMigrations — legacy __cachink_migrations tracker', () => {
  it('adopts the old tracker, applies only 0004 and keeps sync state', async () => {
    ...
    expect(
      sqlite.prepare("SELECT value FROM __xangarro_sync_state WHERE scope = 'localPushHwm'").get(),
    ).toEqual({
      value: '7',
    });
  });
  ...
  it('is idempotent on the second launch after the upgrade', async () => {
    ...
    await runMigrations(db);
    await runMigrations(db);
    expect(tags(sqlite, '__xangarro_migrations')).toHaveLength(PRE_RENAME.length + 1);
  });
  ...
  it('does not create or rename a legacy tracker on a fresh install', async () => {
    ...
    expect(hasTable(sqlite, '__cachink_migrations')).toBe(false);
    expect(tags(sqlite, '__xangarro_migrations')).toContain('0004_xangarro_sync_tables');
  });
