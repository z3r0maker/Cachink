...
export class DrizzleConversionsRepository implements ConversionsRepository {
  ...
  async create(input: CreateConversionInput): Promise<Conversion> {
    const id = newEntityId<ConversionId>();
    const ts = now();
    const row = {
      id,
      ...
      movimientoSalidaId: input.movimientoSalidaId,
      movimientoEntradaId: input.movimientoEntradaId,
  ...
  async findById(id: ConversionId): Promise<Conversion | null> {
    const r = await this.#db
      .select()
      .from(conversions)
      .where(and(eq(conversions.id, id), isNull(conversions.deletedAt)))
      .get();
  ...
  async findByBusiness(businessId: BusinessId): Promise<readonly Conversion[]> {
    const rows = await this.#db
      .select()
      .from(conversions)
      .where(and(eq(conversions.businessId, businessId), isNull(conversions.deletedAt)))
      .all();
  ...
  #map(r: Row): Conversion {
    return {
      id: r.id as ConversionId,
      recetaId: r.recetaId as ConversionRecetaId,
      materiaPrimaId: r.materiaPrimaId as ProductId,
      productoResultanteId: r.productoResultanteId as ProductId,
      cantidadOrigenUsada: r.cantidadOrigenUsada,
      cantidadResultanteCreada: r.cantidadResultanteCreada,
