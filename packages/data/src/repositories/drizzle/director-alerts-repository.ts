...
import type {
  CreateDirectorAlertInput,
  DirectorAlertsRepository,
} from '../director-alerts-repository.js';
...
export class DrizzleDirectorAlertsRepository implements DirectorAlertsRepository {
  ...
  async findById(id: DirectorAlertId): Promise<DirectorAlert | null> {
    const r = await this.#db
      .select()
      .from(directorAlerts)
      .where(and(eq(directorAlerts.id, id), isNull(directorAlerts.deletedAt)))
      .get();
  ...
  async findUnread(businessId: BusinessId): Promise<readonly DirectorAlert[]> {
    const rows = await this.#db
      ...
      .where(
        and(
          eq(directorAlerts.businessId, businessId),
          eq(directorAlerts.read, false),
          isNull(directorAlerts.deletedAt),
        ),
      )
  ...
  async markAllRead(businessId: BusinessId): Promise<void> {
    await this.#db
      .update(directorAlerts)
      .set({ read: true, updatedAt: now() })
      .where(and(eq(directorAlerts.businessId, businessId), eq(directorAlerts.read, false)))
      .run();
  }

  #map(r: Row): DirectorAlert {
    return {
      id: r.id as DirectorAlertId,
