...
export class DrizzleEntregasCreditoRepository implements EntregasCreditoRepository {
  ...
  async create(input: CreateEntregaCreditoInput): Promise<EntregaCredito> {
    const id = newEntityId<EntregaCreditoId>();
    const ts = now();
    const row = {
      id,
  ...
  async findById(id: EntregaCreditoId): Promise<EntregaCredito | null> {
    const r = await this.#db
      .select()
      .from(entregasCredito)
      .where(and(eq(entregasCredito.id, id), isNull(entregasCredito.deletedAt)))
      .get();
  ...
  async findByClient(clientId: ClientId): Promise<readonly EntregaCredito[]> {
    const rows = await this.#db
      .select()
      .from(entregasCredito)
      .where(and(eq(entregasCredito.clienteId, clientId), isNull(entregasCredito.deletedAt)))
      .all();
  ...
  async findByBusiness(businessId: BusinessId): Promise<readonly EntregaCredito[]> {
    const rows = await this.#db
      .select()
      .from(entregasCredito)
      .where(and(eq(entregasCredito.businessId, businessId), isNull(entregasCredito.deletedAt)))
      .all();
    return rows.map((r) => this.#map(r));
  }
  ...
  #map(r: Row): EntregaCredito {
    return {
      id: r.id as EntregaCreditoId,
      clienteId: r.clienteId as ClientId,
