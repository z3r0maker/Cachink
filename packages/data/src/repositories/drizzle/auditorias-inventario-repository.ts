...
export class DrizzleAuditoriasInventarioRepository implements AuditoriasInventarioRepository {
  ...
  async create(input: CreateAuditoriaInput): Promise<AuditoriaInventario> {
    const id = newEntityId<AuditoriaInventarioId>();
    const ts = now();
    const row = {
      id,
      fecha: input.fecha,
  ...
  async findById(id: AuditoriaInventarioId): Promise<AuditoriaInventario | null> {
    const r = await this.#db
      ...
      .where(and(eq(auditoriasInventario.id, id), isNull(auditoriasInventario.deletedAt)))
      .get();
  ...
  async findLatest(businessId: BusinessId): Promise<AuditoriaInventario | null> {
    const r = await this.#db
      ...
      .where(
        and(
          eq(auditoriasInventario.businessId, businessId),
          isNull(auditoriasInventario.deletedAt),
        ),
      )
      .orderBy(desc(auditoriasInventario.createdAt))
      .limit(1)
      .get();
    return r ? this.#map(r) : null;
  }
  ...
  async findByDateRange(
    ...
    const rows = await this.#db
      ...
      .where(
        and(
          eq(auditoriasInventario.businessId, businessId),
          gte(auditoriasInventario.fecha, from),
          lte(auditoriasInventario.fecha, to),
          isNull(auditoriasInventario.deletedAt),
        ),
      )
      .orderBy(desc(auditoriasInventario.createdAt))
  ...
  async update(id: AuditoriaInventarioId, patch: AuditoriaPatch): Promise<AuditoriaInventario> {
    ...
    if (patch.totalDiscrepancias !== undefined)
      set['totalDiscrepancias'] = patch.totalDiscrepancias;
    if (patch.productosContados !== undefined) set['productosContados'] = patch.productosContados;
    await this.#db
      .update(auditoriasInventario)
      .set(set)
      .where(eq(auditoriasInventario.id, id))
      .run();
    const r = await this.#db
      .select()
      .from(auditoriasInventario)
  ...
  #map(r: Row): AuditoriaInventario {
    return {
      id: r.id as AuditoriaInventarioId,
      fecha: r.fecha as IsoDate,
      estado: r.estado as AuditoriaEstado,
      lineas: r.lineas,
      totalDiscrepancias: r.totalDiscrepancias,
      totalProductos: r.totalProductos,
      productosContados: r.productosContados,
      businessId: r.businessId as BusinessId,
      deviceId: r.deviceId as DeviceId,
      createdByUserId: (r.createdByUserId ?? null) as UserId | null,
      createdAt: r.createdAt as IsoTimestamp,
      updatedAt: r.updatedAt as IsoTimestamp,
