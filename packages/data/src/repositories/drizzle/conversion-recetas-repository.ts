...
export class DrizzleConversionRecetasRepository implements ConversionRecetasRepository {
  ...
  async create(input: CreateConversionRecetaInput): Promise<ConversionReceta> {
    ...
    const row = {
      ...
      cantidadResultante: input.cantidadResultante,
  ...
  async findById(id: ConversionRecetaId): Promise<ConversionReceta | null> {
    const r = await this.#db
      .select()
      .from(conversionRecetas)
      .where(and(eq(conversionRecetas.id, id), isNull(conversionRecetas.deletedAt)))
      .get();
  ...
  async findByMateriaPrima(mpId: ProductId): Promise<readonly ConversionReceta[]> {
    const rows = await this.#db
      .select()
      .from(conversionRecetas)
      .where(and(eq(conversionRecetas.materiaPrimaId, mpId), isNull(conversionRecetas.deletedAt)))
      .all();
  ...
  async findByProductoResultante(prodId: ProductId): Promise<ConversionReceta | null> {
    const r = await this.#db
      ...
      .where(
        and(
          eq(conversionRecetas.productoResultanteId, prodId),
          isNull(conversionRecetas.deletedAt),
        ),
      )
      .get();
    return r ? this.#map(r) : null;
  ...
  async findAllByBusiness(businessId: BusinessId): Promise<readonly ConversionReceta[]> {
    const rows = await this.#db
      .select()
      .from(conversionRecetas)
      .where(and(eq(conversionRecetas.businessId, businessId), isNull(conversionRecetas.deletedAt)))
      .all();
    return rows.map((r) => this.#map(r));
  }
  ...
  async delete(id: ConversionRecetaId): Promise<void> {
    const ts = now();
    await this.#db
      .update(conversionRecetas)
      .set({ deletedAt: ts, updatedAt: ts })
      .where(eq(conversionRecetas.id, id))
      .run();
  }

  #map(r: Row): ConversionReceta {
    return {
      id: r.id as ConversionRecetaId,
      materiaPrimaId: r.materiaPrimaId as ProductId,
