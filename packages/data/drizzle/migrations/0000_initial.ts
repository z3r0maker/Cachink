 ...
 *   - 18 incremental migrations carried DDL inconsistencies from
 *     ALTER TABLE limitations (e.g. TEXT on money columns).
 *   - Money columns use INTEGER affinity (not Drizzle's default NUMERIC)
 *     to match the integer-centavos convention; the DDL override is intentional.
 *   - A single CREATE TABLE per entity is cleaner, faster, and correct.
 *
 * Schema hardening (2026-05-16):
 *   - Tables reordered for FK dependency resolution.
 *   - REFERENCES clauses added to all scalar FK columns.
 *   - CHECK constraints on all enum/domain-value columns.
 *   - UNIQUE constraints on day_closes and conversion_recetas.
 *
 * Contents:
 *   §1  Business entity tables (20 tables, FK-dependency order)
 *   §2  Sync infrastructure (__cachink_change_log, __cachink_sync_state,
 *       __cachink_conflicts)
 *   §3  Change-log triggers (AFTER INSERT + AFTER UPDATE per synced table)
 *   §4  Partial indexes for hot-path queries
 *   §5  Composite index for sync pagination
 */

export const migration0000Sql = `
-- ============================================================
