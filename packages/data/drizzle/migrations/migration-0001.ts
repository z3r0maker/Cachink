...
export const migration0001Sql = `CREATE TABLE IF NOT EXISTS \`__cachink_change_log\` (
...
CREATE INDEX IF NOT EXISTS \`idx_cachink_change_log_id\` ON \`__cachink_change_log\` (\`id\`);
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS \`__cachink_sync_state\` (
\t\`scope\` text PRIMARY KEY NOT NULL,
\t\`value\` text NOT NULL
);
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS \`__cachink_conflicts\` (
\t\`id\` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
...
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS \`idx_cachink_conflicts_detected_at\` ON \`__cachink_conflicts\` (\`detected_at\`);
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_sales_ai\` AFTER INSERT ON \`sales\`
BEGIN
...
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_sales_au\` AFTER UPDATE ON \`sales\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('sales', NEW.id, NEW.updated_at, NEW.device_id, 'update');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_expenses_ai\` AFTER INSERT ON \`expenses\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('expenses', NEW.id, NEW.updated_at, NEW.device_id, 'insert');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_expenses_au\` AFTER UPDATE ON \`expenses\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('expenses', NEW.id, NEW.updated_at, NEW.device_id, 'update');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_products_ai\` AFTER INSERT ON \`products\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('products', NEW.id, NEW.updated_at, NEW.device_id, 'insert');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_products_au\` AFTER UPDATE ON \`products\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('products', NEW.id, NEW.updated_at, NEW.device_id, 'update');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_inventory_movements_ai\` AFTER INSERT ON \`inventory_movements\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('inventory_movements', NEW.id, NEW.updated_at, NEW.device_id, 'insert');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_inventory_movements_au\` AFTER UPDATE ON \`inventory_movements\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('inventory_movements', NEW.id, NEW.updated_at, NEW.device_id, 'update');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_employees_ai\` AFTER INSERT ON \`employees\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('employees', NEW.id, NEW.updated_at, NEW.device_id, 'insert');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_employees_au\` AFTER UPDATE ON \`employees\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
...
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_client_payments_ai\` AFTER INSERT ON \`client_payments\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('client_payments', NEW.id, NEW.updated_at, NEW.device_id, 'insert');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_client_payments_au\` AFTER UPDATE ON \`client_payments\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('client_payments', NEW.id, NEW.updated_at, NEW.device_id, 'update');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_day_closes_ai\` AFTER INSERT ON \`day_closes\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('day_closes', NEW.id, NEW.updated_at, NEW.device_id, 'insert');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_day_closes_au\` AFTER UPDATE ON \`day_closes\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('day_closes', NEW.id, NEW.updated_at, NEW.device_id, 'update');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_recurring_expenses_ai\` AFTER INSERT ON \`recurring_expenses\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('recurring_expenses', NEW.id, NEW.updated_at, NEW.device_id, 'insert');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_recurring_expenses_au\` AFTER UPDATE ON \`recurring_expenses\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('recurring_expenses', NEW.id, NEW.updated_at, NEW.device_id, 'update');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_businesses_ai\` AFTER INSERT ON \`businesses\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
\tVALUES ('businesses', NEW.id, NEW.updated_at, NEW.device_id, 'insert');
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS \`trg_businesses_au\` AFTER UPDATE ON \`businesses\`
BEGIN
\tINSERT INTO \`__cachink_change_log\` (\`table_name\`, \`row_id\`, \`row_updated_at\`, \`row_device_id\`, \`op\`)
